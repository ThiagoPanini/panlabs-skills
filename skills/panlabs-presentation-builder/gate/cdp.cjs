...
// disk", never a node_modules tree.
//
// THIS IS THE ONE THING THE v1 LEFT THAT IS WORTH INHERITING (#208). It was
// written for the v1 render gate (#93, #157), it never knew anything about
// that engine's markup, and it now lives inside the skill because the v2
// gate runs as an invocation check rather than as a maintainer's suite.
//
// Navigation is caller-driven and reload-based: whatever query params the
// stage understands are baked into the URL by the caller. There is no
// key-press navigation here -- walking a deck with ArrowRight and hoping the
// compositor kept up is the mechanism #105 found broken.
