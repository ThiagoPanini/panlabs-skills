...
(async () => {
  ...
  for (const [nome, esperado] of Object.entries(ESPERADO)) {
    const modelo = JSON.parse(fs.readFileSync(path.join(Q22, 'modelo', `${nome}.json`), 'utf8'));
   ...
   */
  const controle = JSON.parse(fs.readFileSync(path.join(Q22, 'modelo', 'web-dados.json'), 'utf8'));
