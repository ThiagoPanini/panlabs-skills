...
const dispor = require(path.join(RAIZ, 'motor', 'dispor.cjs'));
const resolverMod = require(path.join(RAIZ, 'motor', 'resolver.cjs'));
const { validar } = require(path.join(RAIZ, 'motor', 'validar.cjs'));
const ESQUEMA = JSON.parse(fs.readFileSync(path.join(RAIZ, 'esquema.json'), 'utf8'));

const res = resolverMod.criar(require(path.join(RAIZ, 'tema', 'tema.cjs')).carregar('claro'));
const cat = res.cat;
...
function carregar(nome, dir = 'modelo') {
  const m = JSON.parse(fs.readFileSync(path.join(RAIZ, dir, `${nome}.json`), 'utf8'));
...
{
  const m = carregar('subnet-vazia', path.join('modelo', 'recusa'));
  const d = derivar(m, { cat });
