#!/usr/bin/env python3
"""THE INSTALLATION, MEASURED. Six families.

    workbench/panlabs-presentation-builder/tests/check-install.py
...
Nothing announces that. The skill just stops being in the harness.
  ...
  2  link-shape       `~/.agents/…` carries the absolute path and `~/.claude/…`
                      ...
                      two places to repoint on the day the repository moves
  ...
  4  runs-from-both   the DOCUMENTED command runs from each installed path and
                      ...
                      missing `compiler/` reads its front door perfectly
  ...
  6  detects-real-worktree
                      a worktree that lives nowhere near `.claude/worktrees/`
                      is still recognised, because only asking git what the
                      tree IS can see one

THE FONT IS WHAT FAMILY 4 ASSERTS, AND THE THEME IS CHOSEN FOR IT. `base` wears
the machine's own faces on purpose, so a page built in it carries no `@font-face`
and proves nothing about what came through the link. `panlabs` embeds two
subset `.woff2` files, and they are the heaviest thing in the tree -- exactly
what a partial copy or a link into a half-checked-out worktree loses first.

EVERYTHING IS HERMETIC, and it has to be. `HOME` is a scratch directory, so the
machine's real `~/.claude/skills/` is never read and never written -- with
parallel sessions the doctrine here, a check that repointed the live skill homes
would hand every other session a different skill for as long as it ran. And the
tree being installed is a COPY, staged outside any worktree path, so what the
families measure is this branch's skill rather than whatever the main checkout
happens to hold.
"""
import os
import pathlib
...
def stage(dst, under_worktree=False):
    ...
    a ruler leaves no trace on its subject, and carrying the subject's bytecode
    into the copy would make the copy differ from the tree.
    """
...
# --------------------------------------------------------------------------
# the six families, each over one already-installed scratch HOME
# --------------------------------------------------------------------------
...
def check_link_shape(home=None, **_):
    ...
    if not (carrier.is_symlink() and pointer.is_symlink()):
        return False, ("one of the two links is missing, so their shape cannot "
                       "be read -- run `bash tools/install.sh`")
...
def check_not_a_worktree(home=None, **_):
    ...
    if caught:
        return False, (f"link(s) into a worktree: {'; '.join(caught)} -- a "
                       f"worktree is deleted with the session that made it and "
                       f"the skill then vanishes with no warning. Reinstall "
...
def check_runs_from_both(home=None, scratch=None, **_):
    """The documented command, run THROUGH each link.

    NO `node` ON THE PATH, and thirty-five seconds saved per build by it: with
    no browser to find, the render gate degrades to the named SKIP #209 built
    it to degrade to. What this family is about is whether the skill runs
    through a link at all; layer 2 is where a render verdict is asserted.
    """
    ...
    for i, p in enumerate(links(home)):
        ...
        r = subprocess.run(
            [sys.executable, str(p / "compiler/build.py"), str(p / SOURCE),
             str(out), "--theme", THEME],
            capture_output=True, text=True, env=env, timeout=180)
        if r.returncode != 0:
            ...
            return False, (f"the documented command does not run from {p}: "
                           ...
                           f"Reinstall from a checkout that carries compiler/ "
                           f"and examples/")
        if EMBEDDED_FACE not in out.read_text(encoding="utf-8"):
            return False, (f"built from {p} in the {THEME} theme, but the page "
                           f"carries no embedded font -- it would need the "
                           f"network to look right. Check that "
                           f"themes/{THEME}/fonts/ came through the link")
...
def check_refuses_orphan(root=None, **_):
    ...
    silently -- or to refuse. This asserts it refuses, and that it wrote nothing
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # copy is overwritten with it: the plant in the proof edits `root`, and
        # without this the fake worktree would carry a pristine installer and
        # the plant would measure nothing.
        if root is not None:
...
def _repo(cwd, *args):
    """A version-control command, in a staged repository of this check's own."""
...
def check_detects_real_worktree(root=None, **_):
    ...
    family stages a real repository with a real linked worktree, puts a copy of
    the skill in both, and installs from the WORKTREE one. The links must come
    out pointing at the main checkout's copy.
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        repo.mkdir()
        home.mkdir()
        _repo(repo, "init", "-q", "-b", "main")
        (repo / "seed").write_text("x", encoding="utf-8")
        _repo(repo, "add", "-A")
        _repo(repo, "commit", "-qm", "seed")
        r = _repo(repo, "worktree", "add", "-q", "-b", "wt", str(wt))
        if not wt.exists():
            return False, (f"could not stage a real worktree, so this family "
                           f"measured nothing: {(r.stderr or r.stdout).strip()} "
                           f"-- install git, or run this suite where "
                           f"`git worktree` works")
        ...
        return False, (f"installed from a real worktree at {wt} and the links "
                       ...
                       f"`--git-common-dir`) instead of reading the path, "
                       f"because a worktree can live anywhere. "
...
def main():
    ...
    if not (SKILL / "tools/install.sh").exists():
        print("  FAIL installer            tools/install.sh is not there -- "
              "every family below measures what it does, and their verdicts "
              "are unavailable, not false")
        return 1
    with tempfile.TemporaryDirectory(prefix="panlabs-install-check.") as tmp:
        ...
        r = install(root, home)
        if r.returncode != 0:
            said = r.stdout.strip().splitlines()
            print(f"  FAIL installer            it refused to install into a "
                  f"clean scratch HOME: "
                  f"{said[-1] if said else r.stderr.strip()}")
            return 1
