...
const cel = (id, label, style, parent = '1') =>
...
const CASES = [
  {
    ...
    rule: 'A7.1',
    layoutPlan: { background: '#FFFFFF', celulas: [
      cel('sub', 'Private subnet', GROUP + 'strokeColor=#00A4A6;fillColor=#2A3A3A;fontColor=#232F3E;fontSize=12;') ] },
  },
  {
    ...
    rule: 'A7.1',
    layoutPlan: { background: '#FFFFFF', celulas: [
      cel('sub', 'Private subnet', GROUP + 'strokeColor=#00A4A6;fillColor=#1A1A1A;fontColor=#FFFFFF;fontSize=12;'),
      cel('lambda', 'Processar pedido', ICON + 'fillColor=#ED7100;strokeColor=#FFFFFF;fontColor=#232F3E;fontSize=12;', 'sub') ] },
  },
  {
    ...
    rule: 'A7.2',
    layoutPlan: { background: '#F2F3F5', celulas: [
      cel('reg', 'us-east-1', GROUP + 'strokeColor=#00A4A6;fillColor=none;fontColor=#232F3E;fontSize=12;') ] },
  },
  {
    ...
    rule: 'A7.1',
    layoutPlan: { background: '#FFFFFF', celulas: [
      cel('sub', 'Private subnet', GROUP + 'strokeColor=#00A4A6;fillColor=#3A3A3A;fontColor=#FFFFFF;fontSize=12;'),
  ...
  },
  {
    ...
    rule: 'A7.2',
    layoutPlan: { background: '#FFFFFF', celulas: [
      cel('svc', 'Serviço', ICON + 'fillColor=#EEEEEE;strokeColor=#FFFFFF;fontColor=#232F3E;fontSize=12;') ] },
  },
];
...
const CLEAN = {
  ...
  layoutPlan: { background: '#FFFFFF', celulas: [
    cel('sub', 'Private subnet', GROUP + 'strokeColor=#00A4A6;fillColor=#E6F6F6;fontColor=#232F3E;fontSize=12;'),
    cel('lambda', 'Processar pedido', ICON + 'fillColor=#ED7100;strokeColor=#FFFFFF;fontColor=#232F3E;fontSize=12;', 'sub'),
...
function main() {
  ...
  for (const caso of CASES) {
    const r = measure(caso.layoutPlan);
  ...
  const limpo = measure(CLEAN.layoutPlan);
  const ok = limpo.ok;
  if (!ok) falhou = 1;
  console.log(`  ${ok ? '✓' : '✗'} ${CLEAN.name.padEnd(48)} —     um portão que só sabe dizer não não é portão`);
  if (!ok) for (const l of require('../engine/contrast.cjs').summarize(limpo)) console.log('      ' + l);

  console.log(falhou ? '\n  PORTÃO NÃO SABE FALHAR' : `\n  ✓ o portão pega os ${CASES.length} planos ruins e aprova o bom`);
  process.exit(falhou);
}
