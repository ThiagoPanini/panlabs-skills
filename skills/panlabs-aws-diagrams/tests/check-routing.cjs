 ...
 * de propósito — que é o mesmo contrato da quarentena do `check-bons.cjs`.
 */
 ...
 *
 * Ela nasce da sessão do #14 — `varejo-logica` aprovada, `varejo-elaboracao`
...
function vistaTecnica() {
  ...
  return projetar(elaborar(aprovado, ler('varejo-elaboracao.json')), 'tecnica').modelo;
}
...
function ocorrencias(laudo, id) {
  const x = [...laudo.resultados, ...laudo.extras].find(r => r.id === id);
  return x ? { n: x.ocorrencias.length, estado: x.estado, det: x.ocorrencias.map(o => o.o_que) } : null;
}
 ...
 */
function primitiva() {
  const { corredorLivre } = require(path.join(RAIZ, 'motor', 'dispor.cjs'));
  const caixa = (ini, fim, lo, hi) => ({ ini, fim, lo, hi });

  // as três colunas do `web-fluxo-3-az`, na faixa que a perna atravessa
  const grade = [caixa(48, 248, 0, 800), caixa(339, 539, 0, 800), caixa(630, 830, 0, 800)];

  const casos = [
    { nome: 'preferência já livre passa intacta',
      r: corredorLivre([100, 300], grade, 584.5), espera: 584.5 },
    { nome: 'preferência DENTRO de coluna cai no vão vizinho',
      r: corredorLivre([100, 300], grade, 538), espera: 584.5 },
    { nome: 'e escolhe o vão do lado da origem, não o mais largo',
      r: corredorLivre([100, 300], grade, 350), espera: 293.5 },
    { nome: 'obstáculo que não cruza a faixa não conta',
      r: corredorLivre([0, 50], [caixa(339, 539, 100, 800)], 400), espera: 400 },
    { nome: 'rente à borda não é atravessar',
      r: corredorLivre([100, 300], grade, 539), espera: 539 },
    { nome: 'sem obstáculo nenhum devolve a preferência',
    ...
    { nome: 'tudo bloqueado sai pela margem mais perto',
      r: corredorLivre([100, 300], [caixa(0, 1000, 0, 800)], 400), espera: -24 },
    { nome: 'e pela margem da DIREITA quando a preferência está desse lado',
  ...
  for (const c of casos) {
    ...
    if (!ok) falhou = 1;
    console.log(`  ${ok ? '✓' : '✗'} ${c.nome}` + (ok ? '' : `  — esperava ${c.espera}, veio ${c.r}`));
  }
  return falhou;
}
...
async function main() {
  ...
  const corpus = fs.readdirSync(path.join(RAIZ, 'modelo')).filter(f => f.endsWith('.json')).sort();
  const entradas = [
    ...corpus.map(f => ({ nome: path.basename(f, '.json'),
      modelo: JSON.parse(fs.readFileSync(path.join(RAIZ, 'modelo', f), 'utf8')) })),
    { nome: 'vista técnica (sessão do #14)', modelo: vistaTecnica() },
  ];
  ...
  for (const { nome, modelo } of entradas) {
    ...
    for (const p of [r.plano, ...r.paginas]) {
      const a55 = ocorrencias(validarGeometria(p), 'A5.5');
      if (!a55) { falhou = 1; console.log(`  ‼ ${nome}: A5.5 não rodou`); continue; }
      if (!a55.n) continue;
      falhou = 1; travessias += a55.n;
      console.log(`  ✗ ${nome} · página "${p.id}": A5.5 ×${a55.n}`);
  ...
  // `varejo-300-lojas-tecnica-processamento` ainda carregava `A3.4` ×1. Medir a
  ...
  const laudos = paginas.map(p => ({ pagina: p.id, laudo: validarGeometria(p) }));

  for (const [id, teto] of Object.entries(ORCAMENTO_TECNICA)) {
    let total = 0, faltou = false;
    const det = [];
    for (const { pagina, laudo: l } of laudos) {
      const x = ocorrencias(l, id);
      if (!x) { faltou = true; console.log(`  ‼ ${id} não rodou em "${pagina}"`); continue; }
      total += x.n;
      for (const o of x.det) det.push(`${pagina}: ${o}`);
    }
   ...
   */
  const a51 = ocorrencias(laudo, 'A5.1');
  const medida = [...laudo.resultados].find(r => r.id === 'A5.1');
  const dentro = a51 && a51.estado !== 'falha';
  if (!dentro) falhou = 1;
  console.log(`  ${dentro ? '✓' : '✗'} A5.1 ${medida ? `${medida.medida.cruzamentos} cruzamento(s), orçamento ${medida.medida.orcamento_de_falha}` : '—'}` +
    ` → ${a51 ? a51.estado : 'não rodou'}`);
