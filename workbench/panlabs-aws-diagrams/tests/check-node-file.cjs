...
(async () => {
  ...
  for (const [name, expected] of Object.entries(EXPECTED)) {
    const model = JSON.parse(fs.readFileSync(path.join(WORKBENCH, 'models', `${name}.json`), 'utf8'));
   ...
   */
  const control = JSON.parse(fs.readFileSync(path.join(WORKBENCH, 'models', 'web-data.json'), 'utf8'));
