...
//
// A straight port of `proof_driver.py` -- not a require() of it, because Node
// cannot import a Python module. The render gate (#209) is the first Node
// proof in the v2, and the second one to ever exist in this skill's suite:
// the v1's own `check-render.proof.cjs` carried an identical port before it
// went out with the engine it measured (#208). If a second Node proof ever
// lands here, IT reads this file rather than copying it, for exactly the
// reason `proof_driver.py`'s own docstring gives: two copies of a rule that
// drift is the failure mode this whole suite exists to refuse.
//
...
//
// The file has no hyphen in its name because it is required, not run. Every
