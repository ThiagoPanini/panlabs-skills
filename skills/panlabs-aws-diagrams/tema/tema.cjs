...
function montar(bruto, t) {
  ...
  const api = {
    ...
    servico(style, entrada) {
       ...
       */
      if (entrada && PALETAS_MONO.has(entrada.palette) && t.fundo === 'escuro')
     ...
     * duas pontas. Quem confere é `tests/check-tokens-do-12.cjs`.
     *
     * E é por isso que fica aqui e não em `planejar.cjs`: com o hex lá dentro, o
     ...
     */
    ou: () => `text;html=1;fontSize=${t.texto.grupo + 1};fontStyle=1;fontColor=${t.tinta.forte};` +
      `fontFamily=${t.texto.familia};align=left;verticalAlign=middle;`,

    /** `E4`/`X3`: a linha do barramento. Sem ponta — quem tem ponta é o stub. */
    barramento: () => `endArrow=none;html=1;strokeColor=${t.aresta.cor};` +
      `strokeWidth=${t.aresta.espessura};`,

    /** O stub perpendicular que entra na conta, e a aresta agregada do `E3`. */
    stub: () => `edgeStyle=orthogonalEdgeStyle;html=1;strokeColor=${t.aresta.cor};` +
      `strokeWidth=${t.aresta.espessura};endArrow=${t.aresta.ponta};` +
      `endFill=${t.aresta.ponta === 'open' ? 0 : 1};endSize=6;fontSize=${t.texto.aresta};` +
      `fontFamily=${t.texto.familia};fontColor=${t.tinta.forte};labelBackgroundColor=${t.tinta.halo};`,
     ...
     */
    habilitador: () => `edgeStyle=orthogonalEdgeStyle;html=1;strokeColor=${t.tinta.fraca};` +
      `strokeWidth=1.4;dashed=1;dashPattern=6 4;endArrow=${t.aresta.ponta};` +
      `endFill=${t.aresta.ponta === 'open' ? 0 : 1};endSize=6;`,
