...
module.exports = function a1(cena) {
  ...
  // ---------------------------------------------------------------- A1.1
  {
    ...
    if (!title) {
      output.push(falha('A1.1', { mensagem: 'o diagrama não tem título', occurrences: [{ o_que: 'meta.titulo vazio ou ausente', ids: [] }] }));
    } else {
  ...
  // ---------------------------------------------------------------- A1.2
  {
    output.push(cena.legend.length
  ...
  // ---------------------------------------------------------------- A1.3
  {
    ...
    output.push(conforme('A1.3', semEntrada, {
  ...
  // ---------------------------------------------------------------- A1.4
  {
    ...
    output.push(conforme('A1.4', casos, { medida: { elementos: nomeaveis.length, sem_nome: casos.length } }));
  }

  // ---------------------------------------------------------------- A1.5
  {
    if (!modelo) output.push(notApplicable('A1.5', 'o plano não carrega o modelo semântico'));
    else {
      const casos = [...nodes, ...grupos].filter(e => !e.tipoSemantico)
        .map(e => ({ o_que: `${name(e)} não tem tipo declarado no modelo`, ids: [e.id] }));
      output.push(conforme('A1.5', casos, { medida: { elementos: nodes.length + grupos.length, sem_tipo: casos.length } }));
    }
  }
  ...
  // ---------------------------------------------------------------- A1.6
  {
    ...
    output.push(edges.length ? conforme('A1.6', casos, { medida: { edges: edges.length, sem_rotulo: casos.length } })
  ...
  // ------------------------------------------------------------ A1.7 e A1.8
  {
    ...
    if (!edges.length) {
      ...
      output.push(conforme('A1.7', bidirecionais, { medida: { edges: edges.length, bidirecionais: bidirecionais.length } }));

      const semSeta = edges.filter(a => pontas(a) < 1)
        .map(a => ({ o_que: `a aresta "${a.id}" não tem seta`, ids: [a.id] }));
      output.push(conforme('A1.8', semSeta, { medida: { edges: edges.length, sem_seta: semSeta.length } }));
    }
  }
  ...
  // ---------------------------------------------------------------- A1.9
  {
    ...
    if (!cat) output.push(notApplicable('A1.9', 'o catálogo de shapes não está disponível para montar a lista de siglas oficiais'));
      ...
      output.push(conforme('A1.9', casos, { medida: { rotulos: textos.length, siglas_sem_explicacao: casos.length } }));
    }
  }

  // ---------------------------------------------------------------- A1.10
  {
    if (!modelo) output.push(notApplicable('A1.10', 'o plano não carrega o modelo semântico'));
    else {
  ...
  // ---------------------------------------------------------------- A1.11
  {
    if (!modelo) output.push(notApplicable('A1.11', 'o plano não carrega o modelo semântico'));
    else {
      const faltando = ['data', 'versao', 'autor'].filter(k => !modelo[k]);
      output.push(conforme('A1.11', faltando.map(k => ({ o_que: `o modelo não declara "${k}"`, ids: [] })), {
  ...
  // ---------------------------------------------------------------- A1.12
  {
    ...
    const classificadas = new Set(['no', 'group', 'band', 'moldura', 'edge', 'oculto']);
    const casos = [];
    ...
    output.push(conforme('A1.12', casos, { medida: { objetos: cena.elementos.length, orfaos: casos.length } }));
  }
