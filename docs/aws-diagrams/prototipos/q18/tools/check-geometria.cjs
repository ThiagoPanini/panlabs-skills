...
async function main() {
  ...
  if (exemplos) entradas = fs.readdirSync(path.join(Q11, 'modelo')).filter(f => f.endsWith('.json')).map(f => path.join(Q11, 'modelo', f));
  if (!entradas.length) {
  ...
  let gerar;
  try { ({ gerar } = require(path.join(Q11, 'motor', 'gerar.cjs'))); }
  catch (erro) {
    console.error(`não consegui carregar o motor do #11 em ${Q11}: ${erro.message}`);
    process.exit(2);
  }
