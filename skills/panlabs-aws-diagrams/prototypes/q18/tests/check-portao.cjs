...
const { emitir, conferirXml } = require(path.join(Q11, 'motor', 'emitir.cjs'));
...

{
  ...
  const modelo = JSON.parse(fs.readFileSync(path.join(Q11, 'modelo', 'pedidos-serverless.json'), 'utf8'));
  gerar(modelo).then(r => {
    let laudo = null;
    let barrou = null;
    try {
