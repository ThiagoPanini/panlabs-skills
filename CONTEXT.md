...
Nem toda verificação é **checagem de invocação** (roda dentro da skill) nem **validação de desenvolvimento** (genérica, vale para qualquer skill) — falta a que testa o motor de UMA skill específica: geometria, determinismo, semântica. Ela não serve a nenhuma outra skill do repositório, e a skill instalada nunca a roda — então não é nem uma nem outra, e ficar dentro da árvore só infla o pacote publicado. Mora num diretório irmão, `workbench/<nome-da-skill>/`, rastreado neste mesmo repositório e apontando **para dentro** da skill que testa — nunca o inverso, porque instalar a skill não instala o irmão. _Evitar_: bancada (sozinha — é só uma parte do que um diretório desses guarda, não o nome dele), corpus de testes (sozinho) — a confusão que motivou o nome é exatamente com as duas categorias acima.

**A primeira instância saiu, e o conceito ficou.** O #44 abriu `workbench/panlabs-aws-diagrams/` com a suíte de 8 camadas e o corpus que ela come; o #45 estendeu com a bancada (`tools/`) e o catálogo de extração/conferência (`catalog/`); o #121 acrescentou a simulação cega (`blind-run/`), que montava o ambiente da corrida do #47 e provava o próprio isolamento. O **#235 retirou a árvore inteira** — 122 arquivos — e com ela as 26 citações que a apontavam de dentro da skill publicada, que o `scripts/checks/references.sh` cobrava por existência. A `panlabs-aws-diagrams` ficou sem régua de motor: o que sobra medindo-a é o portão de estrutura, que não olha o desenho. O material está no histórico, não na árvore. Hoje a única instância viva do conceito é `workbench/panlabs-presentation-builder/`, e foi dela que a forma sobreviveu.
