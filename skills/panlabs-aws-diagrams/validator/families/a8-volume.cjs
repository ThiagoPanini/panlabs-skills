...
module.exports = function a8(cena) {
  ...
  // ---------------------------------------------------------------- A8.1
  {
    ...
    output.push(V <= target ? ok('A8.1', { medida, mensagem: `${V} nó(s) de primeira classe (alvo ≤ ${target})` })
  ...
  // ---------------------------------------------------------------- A8.3
  {
    if (!cena.grau.size) output.push(notApplicable('A8.3', edges.length
