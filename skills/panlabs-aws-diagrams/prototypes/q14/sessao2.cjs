 ...
 *   6. as duas vistas    — no MESMO arquivo, duas paginas
 *
...
async function main() {
  ...
  // 4 e 5 -------------------------------------------------------------------
  const elaboracao = JSON.parse(fs.readFileSync(path.join(__dirname, 'modelo', 'varejo-elaboracao.json'), 'utf8'));
  ...
  const nova = lerPaginas(rl.xml).paginas[0];
  ...
  console.log(`\n  → ${path.relative(process.cwd(), saida)}  (${juntos.length} bytes, 2 paginas)`);
